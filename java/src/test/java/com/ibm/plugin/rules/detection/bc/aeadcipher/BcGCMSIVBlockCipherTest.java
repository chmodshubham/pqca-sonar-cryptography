...
class BcGCMSIVBlockCipherTest extends TestBase {
    @Test
    void test() {
        CheckVerifier.newVerifier()
                .onFile(
                        "src/test/files/rules/detection/bc/aeadcipher/BcGCMSIVBlockCipherTestFile.java")
                .withChecks(this)
    ...
    public void asserts(
        ...
        assertThat(modeNode2).isNotNull();
        assertThat(modeNode2.getChildren()).isEmpty();
        assertThat(modeNode2.asString()).isEqualTo("GCM-SIV");

        // Encrypt under AuthenticatedEncryption
        INode encryptNode2 = authenticatedEncryptionNode2.getChildren().get(Encrypt.class);
        assertThat(encryptNode2).isNotNull();
        assertThat(encryptNode2.getChildren()).isEmpty();
        assertThat(encryptNode2.asString()).isEqualTo("ENCRYPT");

        // TagLength under AuthenticatedEncryption
        INode tagLengthNode2 = authenticatedEncryptionNode2.getChildren().get(TagLength.class);
