...
class ResolveDoubleSymbolReferenceTest extends TestBase {

    @Test
    void test() {
        CheckVerifier.newVerifier()
                .onFile("src/test/files/rules/resolve/ResolveDoubleSymbolReferenceTestFile.java")
                .withChecks(this)
                .verifyIssues();
    }
    ...
    public void asserts(
        ...
        if (findingId == 0) {
            ...
            DetectionStore<JavaCheck, Tree, Symbol, JavaFileScannerContext> wrap =
                    getStoreOfValueType(CipherAction.class, detectionStore.getChildren());
            assertThat(wrap).isNotNull();
            assertThat(wrap.getDetectionValueContext()).isInstanceOf(CipherContext.class);
            assertThat(wrap.getDetectionValues()).hasSize(1);
            value = wrap.getDetectionValues().get(0);
            assertThat(value).isInstanceOf(CipherAction.class);
            assertThat(value.asString()).isEqualTo("WRAP");

            aes1 = getStoreOfValueType(Algorithm.class, wrap.getChildren());
            assertThat(aes1).isNotNull();
            assertThat(aes1.getDetectionValueContext()).isInstanceOf(SecretKeyContext.class);
            assertThat(aes1.getDetectionValues()).hasSize(1);
            value = aes1.getDetectionValues().get(0);
            ...
            keySize = getStoreOfValueType(KeySize.class, aes1.getChildren());
            assertThat(keySize).isNotNull();
            assertThat(keySize.getDetectionValueContext()).isInstanceOf(SecretKeyContext.class);
            assertThat(keySize.getDetectionValues()).hasSize(1);
            value = keySize.getDetectionValues().get(0);
